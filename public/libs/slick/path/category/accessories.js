...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (cachedProducts) {
    renderAcessórios(cachedProducts);
  }
...
function renderAcessórios(products) {
  const container = document.querySelector('.acessórios .produtos-grid');
  if (!container) return;

  const fragment = document.createDocumentFragment();
  const acessórios = products.filter(p => p.category === "Acessórios");
  
  container.innerHTML = "";
  
  // Pré-carrega as primeiras imagens para prioridade
  acessórios.slice(0, 4).forEach(p => {
    const imgUrl = getImageUrl(p);
    preloadImage(imgUrl);
  });

  acessórios.forEach(product => {
    fragment.appendChild(createProductElement(product));
  });

  container.appendChild(fragment);
  adjustLayout(container, acessórios.length);
}
