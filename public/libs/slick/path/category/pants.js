...
function renderCalças(products) {
  const container = document.querySelector('.calças .produtos-grid');
  if (!container) return;

  const fragment = document.createDocumentFragment();
  const calças = products.filter(p => p.category === "Calças");
  
  container.innerHTML = "";
  
  // Pré-carrega as primeiras imagens para prioridade
  calças.slice(0, 4).forEach(p => {
    const imgUrl = getImageUrl(p);
    preloadImage(imgUrl);
  });

  calças.forEach(product => {
    fragment.appendChild(createProductElement(product));
  });

  container.appendChild(fragment);
  adjustLayout(container, calças.length);
}
