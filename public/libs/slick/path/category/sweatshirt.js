...
function renderMoletons(products) {
  const container = document.querySelector('.moletons .produtos-grid');
  if (!container) return;
  ...
  moletons.slice(0, 4).forEach(p => {
    const imgUrl = getImageUrl(p);
    preloadImage(imgUrl);
  });

  moletons.forEach(product => {
    fragment.appendChild(createProductElement(product));
  });

  container.appendChild(fragment);
  adjustLayout(container, moletons.length);
}
