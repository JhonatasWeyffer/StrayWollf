...
function renderCalçados(products) {
  const container = document.querySelector('.calçados .produtos-grid');
  if (!container) return;

  const fragment = document.createDocumentFragment();
  const calçados = products.filter(p => p.category === "Calçados");
  
  container.innerHTML = "";
  
  // Pré-carrega as primeiras imagens para prioridade
  calçados.slice(0, 4).forEach(p => {
    const imgUrl = getImageUrl(p);
    preloadImage(imgUrl);
  });

  calçados.forEach(product => {
    fragment.appendChild(createProductElement(product));
  });

  container.appendChild(fragment);
  adjustLayout(container, calçados.length);
}
