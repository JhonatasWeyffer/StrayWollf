...
function renderRegatas(products) {
  const container = document.querySelector('.regatas .produtos-grid');
  if (!container) return;

  const fragment = document.createDocumentFragment();
  const regatas = products.filter(p => p.category === "Regatas");
  
  container.innerHTML = "";
  
  // Pré-carrega as primeiras imagens para prioridade
  regatas.slice(0, 4).forEach(p => {
    const imgUrl = getImageUrl(p);
    preloadImage(imgUrl);
  });

  regatas.forEach(product => {
    fragment.appendChild(createProductElement(product));
  });

  container.appendChild(fragment);
  adjustLayout(container, regatas.length);
}
